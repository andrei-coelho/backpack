...
class Request {
    ...
    public static function input($key = ""){
        ...
        if($key != "")
            return isset(self::$instance->inputs[$key]) ? 
            self::$instance->inputs[$key] : false;
            
        return count(self::$instance->inputs) > 0 ? self::$instance->inputs : false;

    }
    
    public static function all($key = ""){
        
        $all = array_merge(self::$instance->gets, self::$instance->posts, self::$instance->inputs);
        
        if($key != "")
            return isset($all[$key]) ? 
            $all[$key] : false;
        
        return count($all) > 0 ? $all : false;
    }
